...
// ===========================================================================
// UI: TabControl for source selection (My Repo vs DaxLib)
// ===========================================================================
var tabs = new System.Windows.Forms.TabControl();
tabs.Dock = System.Windows.Forms.DockStyle.Fill;

// Tab 1: My Repo
var tabMyRepo = new System.Windows.Forms.TabPage("My Repo (.dax)");
var treeMyRepo = new System.Windows.Forms.TreeView();
treeMyRepo.CheckBoxes = true;
treeMyRepo.Dock = System.Windows.Forms.DockStyle.Fill;
treeMyRepo.HideSelection = false;
treeMyRepo.FullRowSelect = true;
treeMyRepo.ShowNodeToolTips = true;
tabMyRepo.Controls.Add(treeMyRepo);

// Tab 2: DaxLib
var tabDaxLib = new System.Windows.Forms.TabPage("DaxLib (.tmdl)");
var treeDaxLib = new System.Windows.Forms.TreeView();
treeDaxLib.CheckBoxes = true;
treeDaxLib.Dock = System.Windows.Forms.DockStyle.Fill;
treeDaxLib.HideSelection = false;
treeDaxLib.FullRowSelect = true;
treeDaxLib.ShowNodeToolTips = true;
tabDaxLib.Controls.Add(treeDaxLib);

// Add tabs to TabControl
tabs.TabPages.Add(tabMyRepo);
tabs.TabPages.Add(tabDaxLib);

// Place TabControl into row 0 of layout
...
// ===========================================================================
// Helper: Scan DaxLib repo for .tmdl functions
// ===========================================================================
System.Collections.Generic.List<dynamic> GetDaxLibFuncs(string apiUrl, string relPath)
{
    var list = new System.Collections.Generic.List<dynamic>();
    ...
    try
    {
        ...
        foreach (var item in items)
        {
            ...
            if (type == "dir")
            {
                var nextApi = item["url"]?.ToString();
                var nextRel = string.IsNullOrEmpty(relPath) ? name : $"{relPath}/{name}";
                if (!string.IsNullOrEmpty(nextApi))
                {
                    list.AddRange(GetDaxLibFuncs(nextApi, nextRel));
                }
            }
            else if (type == "file" && name.EndsWith(".tmdl", StringComparison.OrdinalIgnoreCase))
            {
                try
                {
                    var rawUrl = item["download_url"]?.ToString();
                    if (string.IsNullOrEmpty(rawUrl)) continue;

                    var code = client.GetStringAsync(rawUrl).Result;

                    foreach (System.Text.RegularExpressions.Match m in
                        System.Text.RegularExpressions.Regex.Matches(code, @"function\s+'([^']+)'"))
                    {
                ...
                {
                    System.Diagnostics.Debug.WriteLine("Parse .tmdl error: " + innerEx);
                }
            }
        }
    }
    ...
    {
        System.Diagnostics.Debug.WriteLine("GetDaxLibFuncs error: " + ex);
        System.Windows.Forms.MessageBox.Show($"Error scanning daxlib repo: {ex.Message}");
    }

    return list ?? new System.Collections.Generic.List<dynamic>();
}
...
// ===========================================================================
// Helper: Refresh My Repo tree (repo + model-only UDFs)
// ===========================================================================
Action RefreshTreeMyRepo = () =>
{
    WithWaitCursor(form, () =>
    {
        // Load UDFs from your GitHub repo
        var funcs = GetFuncs(GetApiUrl(folder), "");
        ...
        treeMyRepo.BeginUpdate(); // freeze redraw for performance
        try
        {
            treeMyRepo.Nodes.Clear();
            var root = new System.Windows.Forms.TreeNode($"═════ GitHub: {owner}/{repo}/{folder} ═════");
            treeMyRepo.Nodes.Add(root);
            ...
            foreach (var f in funcs.OrderBy(x => x.Path).ThenBy(x => x.Name))
            {
                ...
                // Bold font if function exists in model
                if (existing.Contains(f.Name))
                    node.NodeFont = new System.Drawing.Font(treeMyRepo.Font, System.Drawing.FontStyle.Bold);
            ...
            // Build model-only branch
            var modelOnlyRoot = new System.Windows.Forms.TreeNode("═════ Model-only functions (not in GitHub) ═════");
            treeMyRepo.Nodes.Add(modelOnlyRoot);
            ...
            treeMyRepo.ExpandAll();
        }
        finally
        {
            treeMyRepo.EndUpdate();
        }
    });
};
...
// ===========================================================================
// Helper: Refresh DaxLib tree (.tmdl functions only, no model-only branch)
// ===========================================================================
Action RefreshTreeDaxLib = () =>
{
    WithWaitCursor(form, () =>
    {
        // Load UDFs from DaxLib repo
        var funcs = GetDaxLibFuncs("https://api.github.com/repos/sql-bi/daxlib/contents/packages", "");

        treeDaxLib.BeginUpdate(); // freeze redraw for performance
        try
        {
            treeDaxLib.Nodes.Clear();
            var root = new System.Windows.Forms.TreeNode("═════ DaxLib (.tmdl packages) ═════");
            treeDaxLib.Nodes.Add(root);
            ...
            foreach (var f in funcs.OrderBy(x => x.Path).ThenBy(x => x.Name))
            {
                ...
                foreach (var part in pathParts)
                {
                    // Look for existing folder node or create new one
                    var existingNode = current.Cast<System.Windows.Forms.TreeNode>()
                        .FirstOrDefault(n => n.Text == part);
                    if (existingNode == null)
                    {
                        existingNode = new System.Windows.Forms.TreeNode(part);
                        current.Add(existingNode);
                    }
                    current = existingNode.Nodes;
                }
            ...
            treeDaxLib.ExpandAll();
        }
        finally
        {
            treeDaxLib.EndUpdate();
        }
    });
};
...
var currentTree = (tabs.SelectedTab == tabMyRepo) ? treeMyRepo : treeDaxLib;
TraverseNodes(currentTree.Nodes, node => 
...
// ===========================================================================
// Button: Update Model from Repo or DaxLib
// Supports both .dax (GitHub) and .tmdl (DaxLib)
// ===========================================================================
updateModel.Click += (s, e) =>
{
    ...
    WithWaitCursor(form, () =>
    {
        ...
        foreach (var f in selected)
        {
            try
            {
                // Case 1: Repo (.dax) node
                if (tabs.SelectedTab == tabMyRepo && f.Url != null)
                {
                    var path = GetFilePath(f);
                ...
                // Case 2: DaxLib (.tmdl) node
                else if (tabs.SelectedTab == tabDaxLib && f.Code != null)
                {
                    // Regex: match exactly one function block at a time
                    string funcPattern = $@"(?ms)(?<docs>(?:^///.*\n)*)function\s+'{System.Text.RegularExpressions.Regex.Escape(f.Name)}'\s*=\s*(?<body>[\s\S]*?)(?=^///|^function\s+'|\Z)";

                    var match = System.Text.RegularExpressions.Regex.Match(
                        f.Code,
                        funcPattern,
                        System.Text.RegularExpressions.RegexOptions.Multiline | System.Text.RegularExpressions.RegexOptions.Singleline | System.Text.RegularExpressions.RegexOptions.IgnoreCase
                    );
                    ...
                    if (match.Success)
                    {
                        ...
                        string desc = null;
                        var docs = match.Groups["docs"].Value;
                        if (!string.IsNullOrEmpty(docs))
                        {
                            var docLines = new System.Collections.Generic.List<string>();
                            foreach (var line in docs.Split(new[] { '\n' }, System.StringSplitOptions.RemoveEmptyEntries))
                            {
                                var trimmed = line.Trim();
                                if (trimmed.StartsWith("///"))
                                    docLines.Add(trimmed.Substring(3).Trim());
                            }
                            if (docLines.Count > 0)
                                desc = string.Join("\n", docLines);
                        }

                        // Extract and clean function body
                        var body = match.Groups["body"].Value;
                        var codeLines = body.Split(new[] { '\n' }, System.StringSplitOptions.None);
                        var filteredLines = new System.Collections.Generic.List<string>();
                        foreach (var line in codeLines)
                        {
                            var trimmed = line.TrimStart();
                            if (trimmed.StartsWith("annotation "))
                                continue; // drop annotations
                            if (trimmed.Contains("```"))
                                continue; // drop ``` markers
                            filteredLines.Add(line);
                        }
                        var processed = string.Join("\n", filteredLines).Trim();
                        var normalized = NormalizeLineEndings(processed);

                        // Always overwrite existing function
                        var fn = Model.Functions.FirstOrDefault(x => x.Name.Equals(f.Name, System.StringComparison.OrdinalIgnoreCase));
                        if (fn == null)
                            fn = Model.AddFunction(f.Name, normalized);
                        else
                    ...
                    {
                        var snippet = f.Code.Length > 200 ? f.Code.Substring(0, 200) + "..." : f.Code;
                        System.Diagnostics.Debug.WriteLine($"Regex failed for {f.Name}. First 200 chars: {snippet}");
                        System.Windows.Forms.MessageBox.Show($"Could not extract code for function '{f.Name}' from .tmdl file.");
                    }
                }
            }
            catch (System.Exception ex)
...
// ===========================================================================
// Event: Enable/disable GitHub buttons depending on active tab
// ===========================================================================
tabs.SelectedIndexChanged += (s, e) =>
{
    bool inRepo = (tabs.SelectedTab == tabMyRepo);

    // GitHub buttons only valid in repo tab
    updateGitHub.Enabled = inRepo;
    createGitHub.Enabled = inRepo;
    openGitHub.Enabled   = inRepo;

    // Compare works in both tabs, Update Model works in both
